...
void cannon(double *bufA, double *bufB, double *bufC, double *tmpBufA, double *tmpBufB, double *tmpBufC, int n1, int n2, int n3) {
    ...
    for (i = 0; i < rootNumprocs; i++) {
        ...
        // 为提高效率，异步发送（其实也许就没有上面说的必要了？）
        MPI_Request req[4];
        MPI_Status sta[4];
        int sendToA = procBlockRow * rootNumprocs + (procBlockCol - 1 + rootNumprocs) % rootNumprocs;
        int recvFromA = procBlockRow * rootNumprocs + (procBlockCol + 1) % rootNumprocs;
        // 向上移位需要的内容
        int sendToB = ((procBlockRow - 1 + rootNumprocs) % rootNumprocs) * rootNumprocs + procBlockCol;
        int recvFromB = ((procBlockRow + 1) % rootNumprocs) * rootNumprocs + procBlockCol;
        MPI_Irecv(tmpBufA, maxrows_a*maxcols_a, MPI_DOUBLE, recvFromA, SHIFT_A_ID, MPI_COMM_WORLD, &req[2]);
        MPI_Irecv(tmpBufB, maxrows_b*maxcols_b, MPI_DOUBLE, recvFromB, SHIFT_B_ID, MPI_COMM_WORLD, &req[3]);
        MPI_Isend(bufA, maxrows_a*maxcols_a, MPI_DOUBLE, sendToA, SHIFT_A_ID, MPI_COMM_WORLD, &req[0]);
        MPI_Isend(bufB, maxrows_b*maxcols_b, MPI_DOUBLE, sendToB, SHIFT_B_ID, MPI_COMM_WORLD, &req[1]);
        // 2. 传输已经开始了，可以在此过程中进行运算
